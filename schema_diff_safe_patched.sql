...
BEGIN
  ...
  IF NOT EXISTS (
    ...
    WHERE c.conname = 'achievements_student_id_fkey'
  ...
  -- Validate if present but not yet validated
  IF EXISTS (
    SELECT 1
    FROM pg_constraint c
    JOIN pg_class t ON t.oid = c.conrelid
    ...
    ALTER TABLE "public"."achievements"
      VALIDATE CONSTRAINT "achievements_student_id_fkey";
  END IF;
END $$;
...
BEGIN
  IF NOT EXISTS (
    ...
    JOIN pg_class t ON t.oid = c.conrelid
    JOIN pg_namespace n ON n.oid = t.relnamespace
    WHERE c.conname = 'fk_cvp_tutor'
      AND n.nspname = 'public'
      AND t.relname = 'classvault_purchases'
  ) THEN
    ALTER TABLE "public"."classvault_purchases"
      ADD CONSTRAINT "fk_cvp_tutor"
  ...
  IF EXISTS (
    ...
    ALTER TABLE "public"."classvault_purchases"
      VALIDATE CONSTRAINT "fk_cvp_tutor";
  END IF;
END $$;
...
BEGIN
  IF NOT EXISTS (
    ...
    JOIN pg_class t ON t.oid = c.conrelid
    JOIN pg_namespace n ON n.oid = t.relnamespace
    WHERE c.conname = 'purchases_student_id_fkey'
      AND n.nspname = 'public'
      AND t.relname = 'classvault_purchases'
  ) THEN
    ALTER TABLE "public"."classvault_purchases"
      ADD CONSTRAINT "purchases_student_id_fkey"
      FOREIGN KEY ("student_id") REFERENCES "users"("id")
      ON DELETE SET NULL NOT VALID;
  END IF;
  ...
  IF EXISTS (
    ...
    ALTER TABLE "public"."classvault_purchases"
      VALIDATE CONSTRAINT "purchases_student_id_fkey";
  END IF;
END $$;
