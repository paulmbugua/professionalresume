...
export function normalizeCvDraft(draft = {}) {
  ...
  return {
    ...
    typography: { ...DEFAULT_TYPOGRAPHY, ...(draft.typography || {}) },
    formatting: {
      textColor: '#0f172a',
    ...
    templateTheme: { primary: '#0f172a', ...templateDefaults, ...(draft.templateTheme || {}) },
...
export function resolveTemplateTypography(draft = {}) {
  ...
  const defaults =
    templateTypographyDefaults[templateId] || templateTypographyDefaults['ats-minimal'];
  const user = { ...DEFAULT_TYPOGRAPHY, ...(draft.typography || {}) };
  const hasUserBodyOverride =
    draft?.typography &&
    (draft.typography.bodySize != null || draft.typography.baseFontSize != null);

  const bodyDelta =
    (Number(user.bodySize) || DEFAULT_TYPOGRAPHY.bodySize) - DEFAULT_TYPOGRAPHY.bodySize;
  const baseDelta =
    (Number(user.baseFontSize) || DEFAULT_TYPOGRAPHY.baseFontSize) -
    DEFAULT_TYPOGRAPHY.baseFontSize;
  const headingDelta =
    (Number(user.h1Size) || DEFAULT_TYPOGRAPHY.h1Size) - DEFAULT_TYPOGRAPHY.h1Size;
  ...
  const headline = clamp(
    defaults.headline + effectiveBodyDelta * 0.42 + headingDelta * 0.2,
    11.2,
    19
  );
  const lineHeight = clamp(
    Number(user.lineHeight) || defaults.lineHeight || DEFAULT_TYPOGRAPHY.lineHeight,
    1.28,
    1.65
  );
