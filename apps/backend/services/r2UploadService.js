...
const publicBase = (
  process.env.R2_PUBLIC_BASE_URL_PHOTOS || process.env.R2_PUBLIC_BASE_URL_DOCS || ''
).replace(/\/$/, '');
const uploadExpires = Number(process.env.R2_UPLOAD_EXPIRES_SEC || 300);
export const maxPhotoBytes = Number(process.env.R2_MAX_PHOTO_BYTES || 3 * 1024 * 1024);

const allowedPhotoContentTypes = new Set(['image/jpeg', 'image/png', 'image/webp']);
...
export async function createPhotoUploadPresign({ key, contentType }) {
  ...
  const uploadUrl = await getSignedUrl(r2Client, command, { expiresIn: uploadExpires });
