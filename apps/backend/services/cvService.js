...
export async function createExportRecord({ draftId, userId, fileKey, publicUrl, mimeType, bytes }) {
  const { rows } = await pool.query(
    `INSERT INTO cv_exports (draft_id, user_id, file_key, public_url, mime_type, bytes)
     VALUES ($1,$2,$3,$4,$5,$6)
     RETURNING id, draft_id, user_id, file_key, public_url, mime_type, bytes, created_at`,
...
export async function userCanAccessFileKey(userId, fileKey) {
  const { rows } = await pool.query(
    `SELECT id FROM cv_exports WHERE user_id = $1 AND file_key = $2 LIMIT 1`,
