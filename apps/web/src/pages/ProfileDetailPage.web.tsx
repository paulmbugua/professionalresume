...
import type { TutorProfile } from '@shared/types';  // Updated import

const convertToTutorProfile = (profile: LocalTutorProfile): TutorProfile => ({
  ...profile,
  languages: profile.languages ?? [],
  user: profile.user ?? profile.id, // fallback to id if no user is provided
  pricing: {
...
const ProfileDetailPage = () => {
  ...
  const { backendUrl, profile: myProfile } = useShopContext();
  ...
  // Convert to a TutorProfile (which now has pricing properties as strings)
  const numericProfile = convertToTutorProfile(tutorProfile);
  const statusColor =
  ...
  return (
    <div className="bg-gray-900 text-white min-h-screen relative">
      ...
      <div className="pt-24 p-4 max-w-6xl mx-auto flex flex-col lg:flex-row gap-8">
        {/* Left Column: Profile Image and Video */}
        ...
        <div className="lg:w-1/2 bg-gray-800 p-6 rounded-lg shadow-lg space-y-6">
          ...
          <button
            onClick={() => handleCreateSession(navigate)}
          ...
          {/* Profile Actions for Messaging (using numericProfile.user to ensure a string) */}
          <div>
      ...
      {/* Details Section: About Me and Tutor Reviews */}
      ...
      {/* Recommended Tutors Section with Back Button */}
