          ...
          {chats.length > 0 ? (
            chats.map((chat, index) => (
              <li
                ...
                className={`p-3 rounded-lg cursor-pointer transition ${chat.messages?.some(msg => msg.unread && msg.sender_id !== myProfile.id) ? 'bg-gray-700' : 'bg-gray-800'} hover:bg-gray-700 shadow-sm`}
              >
      ...
      <div className="flex-grow flex flex-col bg-gray-900 md:ml-72">
        <div className="flex items-center justify-between p-4 bg-gray-800 shadow-lg border-b border-gray-700">
          ...
          {activeChat ? (
            <div className="absolute left-16 md:left-20 flex items-center space-x-3">
              <img src={activeChat.avatar || '/default-avatar.png'} alt="Avatar" className="w-8 h-8 rounded-full" />
        ...
        {activeChat && (
          <div className="p-4 bg-gray-800 shadow-xl flex items-center space-x-3 border-t border-gray-700">
            ...
            <button onClick={handleSendMessage} className="bg-pink-500 text-white px-4 py-2 rounded-lg flex items-center shadow-lg hover:bg-pink-600 transition">
