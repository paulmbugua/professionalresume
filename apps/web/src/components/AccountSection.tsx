...
// Optionally define a type for the user to match what you expect.
interface User {
  userId?: string;
  email: string | null;
  tokens: number;
...
const AccountSection = () => {
  const {
    ...
    // Add these if you’ve defined them in the hook:
              ...
              >
                ...
                {!formData.tutorId && (
                  <div className="p-2 bg-yellow-100 border-l-4 border-yellow-500 text-yellow-700 rounded text-sm">
                    <p>
                      ...
                      and click their profile image. Use the <strong>'Create Session'</strong> button for prefilled details.
                    </p>
                  </div>
                )}
                ...
                <div className="space-y-2">
                  ...
                  <select
                    ...
                    onChange={(e) => {
                      // If your session types are defined as a union type,
                      // cast e.target.value accordingly.
                      const sessionType = e.target.value;
                      const sessionCost = String(formData.pricing[sessionType as keyof typeof formData.pricing] || 0);
                  ...
                  >
                    ...
                    {formData.pricing &&
                      Object.entries(formData.pricing).map(
                        ([sessionType, price]: [string, number]) => (
                  ...
                  <input
                    ...
                    value={formData.date}
                    onClick={(e) => (e.target as HTMLInputElement).showPicker?.()} // cast to HTMLInputElement
                    onChange={(e) =>
            ...
            {role === 'student' && (
              <div className="space-y-4 mt-6">
                ...
                {accountDetails.session?.length > 0 ? (
                  accountDetails.session.map((session: any, index: number) => (
                    <div
                      key={session.id}
                    ...
                    >
                      ...
                      {session.status === 'accepted' && session.zoom_links?.length > 0 && (
                        <div className="zoom-links space-y-2">
                          <p className="text-green-500 font-semibold">Zoom Links Created:</p>
                          {session.zoom_links.map((link: string, idx: number) => (
          ...
          <>
            ...
            {role === 'tutor' && (
              <div className="sessions space-y-6">
                ...
                {accountDetails.session?.length > 0 ? (
                  accountDetails.session.map((session: any) => (
        ...
        {activeTab === 'earnings' && (
          <div className="earnings space-y-4">
            ...
            {accountDetails.earning?.length > 0 ? (
              accountDetails.earning.map((earning: any) => (
