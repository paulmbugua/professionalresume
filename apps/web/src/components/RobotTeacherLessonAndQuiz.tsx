  ...
  return (
    <>
      ...
      <section id="classroom" className={`relative z-[0] ${compactPlayer && !showCourseList ? 'mx-auto max-w-5xl' : ''}`}>
        <div className={compactPlayer ? 'rounded-2xl overflow-hidden ring-1 ring-gray-200 bg-white dark:ring-white/10 dark:bg-white/5' : ''} style={compactPlayer ? { maxHeight: '76vh' } : undefined}>
          <ClassroomThemeShell
            ssml={displaySsml}
            lessons={lessonsArr}
            voiceName={voiceName}
      ...
      {quiz?.questions?.length ? (
        <section className="panel p-4">
          ...
          {isOrgFlow ? (
            <div className={`mt-1 text-xs px-2 py-1 rounded ${disableQuiz ? 'bg-red-600/20 text-red-200' : 'bg-white/10 text-white/90'}`}>
          ...
          <div className="space-y-4">
            {quiz.questions.map((q: any, idx: number) => (
              <div key={q.id} className="rounded-xl bg-white ring-1 ring-gray-200 p-3 dark:bg-white/5 dark:ring-white/10">
                <div className="text-sm font-medium mb-2 text-darkText dark:text-white">{idx + 1}. {q.prompt}</div>
          ...
          <div className="mt-4 flex flex-wrap items-center gap-2">
                ...
                try {
                  if (isOrgFlow && assignmentId) {
                    const r = await fetch(`${backendUrl}/api/orgs/attempts/submit`, {
                      ...
                      body: JSON.stringify({ assignmentId, answers: Object.entries(answers).map(([questionId, choiceIndex]) => ({ questionId, choiceIndex })) }),
                    });
            ...
            {grade && (
              <span className="text-sm text-darkText dark:text-white/80">Score: <span className="font-semibold">{grade.scorePct}%</span> (Pass mark {grade.passMark}%)</span>
            )}
                  ...
                  <div className="mt-2 flex flex-wrap items-center gap-2">
                    <button
                      onClick={async () => {
                        try {
                          ...
                          if (sku) { try { await claim(sku.code); } catch { /* ignore claim error */ } }
                        ...
                        } catch (e) { console.error('[org] manual issue failed', e); }
                      }}
                    ...
                    {certUrl && (
                      <>
                        ...
                        {downUrl && (<a href={downUrl} className="btn bg-indigo-600 hover:bg-indigo-500">Download PDF</a>)}
                      </>
                    )}
                  </div>
                            ...
                            <button
                              onClick={async () => {
                                ...
                                try {
                                  ...
                                  setCertUrl(c.url ?? null);
                                  setDownUrl(c.download_url ?? c.downloadUrl ?? c.url ?? null);
                                } catch (e) { console.error('[tokens] claim/generate failed', e); }
                              }}
