      ...
      {currentTab === 'videos' && (
        videosEmpty ? (
          <div className="text-center text-gray-500 py-8">
            ...
            {role === 'tutor' && (
              <button onClick={() => navigate('/class-vault/upload')}
                className="mt-4 px-6 py-2 bg-gray-800 text-white rounded-full hover:bg-gray-700"
              >Upload Your First Class</button>
            )}
          </div>
        ) : (
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-6">
            {filteredVideos.map(video => (
              <div key={video.id} className="bg-white rounded-lg shadow overflow-hidden flex flex-col">
                ...
                <div className="p-4 flex-1 flex flex-col">
                  ...
                  <p className="text-sm text-gray-500 mt-1 flex-1">{video.subject} • Grade {video.grade_level}</p>
                  ...
                  <div className="mt-4 space-x-2">
                    {role === 'tutor' ? (
                      <button onClick={() => handleDelete(video.id)}
                        ...
                        <FontAwesomeIcon icon={faTrash as IconProp} className="mr-1" /> Delete
                      </button>
                      ...
                      <button onClick={() => handleDownload(video.id)}
                        ...
                        <FontAwesomeIcon icon={faDownload as IconProp} className="mr-1" /> Download
                      </button>
                    ) : (
                      <button onClick={() => handlePurchase(video)}
                              className="px-3 py-2 border border-gray-300 rounded hover:bg-gray-50 flex items-center">
                        <FontAwesomeIcon icon={faShoppingCart as IconProp} className="mr-1" /> Purchase
                      </button>
                    )}
                  </div>
                </div>
        ...
        notesEmpty ? (
          <div className="text-center text-gray-500 py-8">
            ...
            {role === 'tutor' && (
              <button onClick={() => navigate('/class-vault/upload')}
                className="mt-4 px-6 py-2 bg-gray-800 text-white rounded-full hover:bg-gray-700"
              >Upload Your First Notes</button>
            )}
          </div>
        ) : (
          filteredPdfRows.flat().map(pdf => (
            <div key={pdf.id} className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-6 mb-6">
              <div className="bg-white rounded-lg shadow p-4 flex flex-col items-center">
                ...
                <p className="text-sm text-gray-700 mt-2">Price: {pdf.price} tokens</p>
                <div className="mt-auto flex space-x-2">
                  {role === 'tutor' ? (
                    ...
                    <button onClick={() => handleDownload(pdf.id)}
                            className="px-3 py-2 border border-gray-300 rounded hover:bg-gray-50">
                      <FontAwesomeIcon icon={faDownload as IconProp} />
                    </button>
                  ) : (
                    <button onClick={() => handlePurchase(pdf as RecordedVideo)}
                            className="px-3 py-2 border border-gray-300 rounded hover:bg-gray-50">
