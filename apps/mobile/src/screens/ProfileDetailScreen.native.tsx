...
const ProfileDetailPage: React.FC = () => {
  ...
  return (
    <View style={tw`bg-gray-900 flex-1 relative`}>
      ...
      <ScrollView
        contentContainerStyle={tw`pt-24 p-4 mx-auto w-full`}
      >
          ...
          <View style={tw`w-full bg-gray-800 p-6 rounded-lg shadow-lg`}>
            ...
            <View style={tw`mt-4 space-y-1`}>
              {pricingSections.map(([label, val]) => (
                <Text
                  key={label}
                  style={tw`text-sm text-gray-300`}
                >
                  {label}:{' '}
            ...
            <View style={tw`flex-row flex-wrap gap-4`}>
              {aboutSections.map(([title, arr]) => (
                <View key={title} style={tw`w-1/2`}>
                  <Text style={tw`text-lg font-semibold text-pink-500`}>
                    {title}
                  </Text>
                  {arr.length > 0 ? (
                    arr.map((item, i) => (
        ...
        <View style={tw`mt-10 w-full px-4`}>
          ...
          <TouchableOpacity
            ...
            style={tw`mt-4`}
          >
            <Text style={tw`text-pink-500 underline`}>
              &larr; Back
            </Text>
          </TouchableOpacity>
        </View>
      ...
      {showChat && (
        <View
          style={tw`absolute bottom-0 right-0 w-full max-w-md bg-gray-800 border-t border-gray-700 z-50 shadow-xl`}
        >
          <ScrollView
            style={tw``}
          ...
          <View style={tw`flex-row items-center p-2 border-t border-gray-600`}>
            ...
            <TouchableOpacity
              ...
              style={tw`bg-pink-500 px-4 py-2 rounded-r`}
            >
