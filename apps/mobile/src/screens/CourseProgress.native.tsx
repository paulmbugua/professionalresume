...
export default function CourseProgress() {
  ...
  const courseId: string | undefined =
    route?.params?.courseId ?? route?.params?.id;
  ...
  const pickerIconColor = colorScheme === 'dark' ? '#ffffff' : '#0d141c';
  const placeholderColor = colorScheme === 'dark' ? '#9ca3af' : '#6b7a8a';
  ...
  const {
    ...
    update,
  } = useCourseProgress(backendUrl, courseIdS, token);
  ...
  const {
    ...
    submit,
    posting,
  } = useCourseReviews(backendUrl, courseIdS, {
    myStudentId: myId,
    token: token ?? '',
  });
  ...
  const activeItem =
    activeWeek == null
      ? null
      : syllabus.find((w) => w.week === activeWeek);
  ...
  const openWatch = (v: { title?: string; url: string }) => {
    ...
    Linking.openURL(webUrl).catch(() =>
      Alert.alert('Could not open video', webUrl),
    );
    Alert.alert(
      'Watching video',
      'After finishing, tap “I’ve finished” to mark it watched.',
    );
  };
  ...
  /* ---------------- Transcript (OER) hooks ---------------- */
  const [downloadingTranscript, setDownloadingTranscript] = useState(false);
  const downloadOerTranscript = useCallback(
    async () => {
      if (!courseIdS) return;
      if (!courseWatchedAll) {
        Alert.alert(
          'Locked',
          `Watch all videos to unlock transcript${
      ...
      try {
        ...
        if (r.status === 404) {
          const rr = await fetch(
            `${backendUrl}/api/oer/transcript/${encodeURIComponent(courseIdS)}`,
            {
              headers: {
                ...(token ? { Authorization: `Bearer ${token}` } : {}),
              },
          ...
          if (trId2 || anyUrl2) {
            await Linking.openURL(
              anyUrl2 ||
                `${backendUrl}/api/transcripts/${encodeURIComponent(
                  trId2!,
                )}/download`,
            );
          } else {
            Alert.alert(
              'Transcript',
              'Generated, but no download link was returned.',
            );
          }
  ...
  /* ---------------- Certificate (OER) hooks ---------------- */
  const [issuingCert, setIssuingCert] = useState(false);
  const generateFreeOerCertificate = useCallback(
    async () => {
      if (!courseIdS) return;
      if (!courseWatchedAll) {
        Alert.alert(
      ...
      try {
        ...
        if (certId || anyUrl) {
          await Linking.openURL(
            anyUrl ||
              `${backendUrl}/api/certificates/${encodeURIComponent(
                certId!,
              )}/download`,
          );
        } else {
  ...
  /* ---------------- Early exits (no hooks below this) ---------------- */

  if (!courseIdS) {
    return (
      <SafeAreaView
        edges={['left', 'right', 'bottom']}
        style={tw`flex-1 bg-[#f3f6fb] dark:bg-[#050b12]`}
      >
        <ScrollView
        ...
        >
          <View
            style={tw`w-full max-w-xl rounded-2xl bg-white dark:bg-[#0b1016] border border-[#e2edf5] dark:border-white/5 p-6`}
          >
            <Text
              style={tw`text-base font-semibold text-red-600 dark:text-red-400 mb-2`}
            >
              Missing course id.
            </Text>
            <Text style={tw`text-sm text-[#4a5b6c] dark:text-white/70`}>
              We couldn’t find which course to open. Please go back and pick a course
              again.
            </Text>
            <View style={tw`mt-4 flex-row justify-end`}>
              <TouchableOpacity
                onPress={() => navigation.goBack()}
                style={tw`rounded-xl h-10 px-4 bg-[#e7edf4] dark:bg-[#172534] justify-center`}
              >
                <Text
    ...
    return (
      ...
      >
        ...
        >
          ...
          >
            <Text
              style={tw`text-base font-semibold text-[#0d141c] dark:text-white mb-1`}
            >
              Loading your course…
            </Text>
            <Text style={tw`text-sm text-[#4a5b6c] dark:text-white/70`}>
              We’re fetching your syllabus and progress.
            </Text>
          </View>
        </ScrollView>
      </SafeAreaView>
      ...
      >
        ...
        >
          ...
          >
            ...
            <Text style={tw`text-sm text-[#4a5b6c] dark:text-white/70`}>
              Course not found. It may have been removed or is temporarily unavailable.
            </Text>
            <View style={tw`mt-4 flex-row justify-end`}>
              <TouchableOpacity
                onPress={() => navigation.goBack()}
      ...
      >
        ...
        >
          ...
          >
            ...
            <Text style={tw`text-sm text-[#4a5b6c] dark:text-white/70`}>
              This course doesn’t have a syllabus yet. Check back later.
            </Text>
            <View style={tw`mt-4 flex-row justify-end`}>
              <TouchableOpacity
                onPress={() => navigation.goBack()}
  ...
  return (
    ...
    >
      <ScrollView
        ref={scrollRef}
        contentContainerStyle={[
          tw`max-w-3xl w-full self-center px-4 py-8 gap-y-10`,
          { paddingBottom: insets.bottom + 96 }, // keep content above FooterNav
        ]}
      >
        {/* Header */}
        <View style={tw`gap-y-2`}>
          <Text
            style={tw`text-2xl font-bold text-[#0d141c] dark:text-white`}
          >
            {selectedCourse?.title ?? 'Course'}
          </Text>
          {!!selectedCourse?.description && (
            <Text
              style={tw`text-sm text-[#4a5b6c] dark:text-white/70`}
            >
              {selectedCourse.description}
            </Text>
          )}
          ...
          {/* Overall progress */}
          <View
            style={tw`mt-3 rounded-2xl p-3 bg-white dark:bg-[#0b1016] border border-[#e2edf5] dark:border-white/5`}
          >
            <View style={tw`flex-row items-center justify-between mb-1`}>
              <Text
                style={tw`text-sm font-medium text-[#4a5b6c] dark:text-white/70`}
              >
                Overall progress
              </Text>
              <Text
            ...
            <View
              style={tw`h-2 w-full rounded-full bg-[#e5eef7] dark:bg-white/10 overflow-hidden mt-1`}
            >
              <View
                style={[
                  tw`h-2 bg-[#3d99f5] dark:bg-[#3d99f5] rounded-full`,
                  { width: `${counts.pct}%` },
                ]}
              />
            </View>

            {/* 🔽 Status chips – high contrast, clearly showing Not Started by default */}
            <View style={tw`flex-row flex-wrap gap-2 mt-3`}>
              <View style={tw`px-2.5 py-1 rounded-full bg-[#e7edf4] dark:bg-white/5`}>
                <Text
                  style={tw`text-xs font-medium text-[#0d141c] dark:text-white`}
                >
                  Not started: {counts.notStarted}
                </Text>
              </View>
              <View style={tw`px-2.5 py-1 rounded-full bg-[#fff6e5] dark:bg-[#3b2a10]`}>
                <Text
                  style={tw`text-xs font-medium text-[#8a5500] dark:text-[#ffd27a]`}
                >
                  In progress: {counts.inProgress}
                </Text>
              </View>
              <View style={tw`px-2.5 py-1 rounded-full bg-[#e6f7f2] dark:bg-[#073528]`}>
                <Text
                  style={tw`text-xs font-medium text-[#0f766e] dark:text-[#6ee7b7]`}
                >
          ...
          <View style={tw`flex-row flex-wrap gap-2 mt-3`}>
            {counts.completed === 0 && counts.inProgress === 0 ? (
              <TouchableOpacity
                onPress={startCourse}
                style={tw`rounded-xl h-10 px-4 bg-[#3d99f5] dark:bg-[#3d99f5] justify-center`}
              >
                <Text
            ...
            {counts.inProgress + counts.notStarted > 0 && (
              <TouchableOpacity
                ...
                style={tw.style(
                  `rounded-xl h-10 px-4 bg-white dark:bg-[#0b1016] border border-[#cedbe8] dark:border-white/10 justify-center`,
                  (suggestedWeek == null ||
                    !watchedAllForWeek(suggestedWeek) ||
            ...
            <TouchableOpacity
              onPress={() => navigation.goBack()}
              style={tw`rounded-xl h-10 px-4 bg-white dark:bg-[#0b1016] border border-[#cedbe8] dark:border-white/10 justify-center`}
            >
              <Text
                style={tw`text-sm font-semibold text-[#0d141c] dark:text-white`}
              >
            ...
            {activeItem ? (
              <>
                <CourseReadingPanel
                  ...
                  status={activeStatus}
                  onSetStatus={(next: Status) =>
                    setStatus(activeWeek as number, next)
                  }
                />
                ...
                {weekVideos.length > 0 && (
                  <View
                    style={tw`rounded-xl border border-[#cedbe8] dark:border-white/10 bg-white dark:bg-[#0b1016] p-3`}
                  >
                    <Text
                      style={tw`text-sm font-semibold mb-2 text-[#0d141c] dark:text-white`}
                    >
                        ...
                        return (
                          ...
                          >
                            ...
                            <View style={tw`flex-row items-center gap-2`}>
                              <Text
                                style={tw.style(
                                  `text-xs`,
                                    ...
                                    : `text-[#49739c] dark:text-[#8fb4e5]`,
                                )}
                              >
                                {done ? 'Watched' : 'Not watched'}
                              </Text>
                              <TouchableOpacity
                                style={tw`h-8 px-3 rounded-lg border border-[#cedbe8] dark:border-white/10 bg-white dark:bg-[#0b1016] justify-center`}
                                onPress={() =>
                                  openWatch({
                                    title: `Video ${i + 1}`,
                                    url: v.url,
                              ...
                              {!done && (
                                <TouchableOpacity
                                  style={tw`h-8 px-3 rounded-lg bg-[#e7edf4] dark:bg-[#172534] justify-center`}
                    ...
                    {!watchedAllActive && (
                      <Text
                        style={tw`mt-2 text-xs text-[#49739c] dark:text-[#8fb4e5]`}
                      >
                ...
                <View style={tw`flex-row flex-wrap items-center gap-2`}>
                  <TouchableOpacity
                    ...
                    style={tw.style(
                      ...
                      syllabus.findIndex((w) => w.week === activeWeek) === 0
                        ...
                        : `bg-white dark:bg-[#0b1016] border border-[#cedbe8] dark:border-white/10`,
                    )}
                  >
                    <Text
                      style={tw`text-sm font-semibold text-[#0d141c] dark:text-white`}
                  ...
                  <TouchableOpacity
                    onPress={goNext}
                    disabled={
                      syllabus.findIndex((w) => w.week === activeWeek) ===
                      syllabus.length - 1
                    }
                    style={tw.style(
                      `rounded-xl h-10 px-4 justify-center`,
              ...
              <View
                style={tw`rounded-2xl border border-[#e2edf5] dark:border-white/10 bg-white dark:bg-[#0b1016] p-4`}
              >
                <Text
                  style={tw`text-sm text-[#4a5b6c] dark:text-white/70`}
                >
        ...
        <View style={tw`gap-y-3`}>
          {syllabus.map((item) => {
            const current: Status =
              (progressByWeek.get(item.week) ?? 'Not Started') as Status;
            const isSuggested = item.week === suggestedWeek;
            const canComplete =
              watchedAllForWeek(item.week) && readAllForWeek(item.week);

            // whether this week has any explicit status saved yet
            const hasExplicitStatus = progressByWeek.has(item.week);
            ...
            return (
              <View
                ...
                onLayout={onLayout}
                style={tw.style(
                  `p-4 rounded-xl bg-white dark:bg-[#0b1016]`,
                  isSuggested
                    ? `border border-[#3d99f5]`
                    : `border border-[#cedbe8] dark:border-white/10`,
                )}
              >
                <View style={tw`mb-2`}>
                  <Text
                  ...
                  <Text
                    style={tw`text-sm text-[#6b7a8a] dark:text-white/60`}
                  >
                    Status: {current}
                    {isSuggested ? ' • current' : ''}
                  </Text>
                ...
                <View style={tw`flex-row items-center flex-wrap gap-2`}>
                  {current === 'Not Started' && (
                    <TouchableOpacity
                      onPress={quickStart}
                      style={tw`rounded-lg h-9 px-3 bg-[#e7edf4] dark:bg-[#172534] justify-center`}
                    >
                      <Text
                        style={tw`text-sm font-semibold text-[#0d141c] dark:text-white`}
                      >
                  ...
                  {current !== 'Completed' && (
                    <TouchableOpacity
                      ...
                      style={tw.style(
                        `rounded-lg h-9 px-3 bg-[#3d99f5] dark:bg-[#3d99f5] justify-center`,
                  ...
                  {/* Status picker */}
                  <View
                    style={tw`border border-[#cedbe8] dark:border-white/10 rounded bg-white dark:bg-[#0b1016]`}
                  >
                    <Picker
                      ...
                      style={tw`h-9 w-44 text-sm text-[#0d141c] dark:text-white`}
                    >
                      {/* Default placeholder-style label when user hasn't touched this week */}
                      <Picker.Item
                        label={
                          hasExplicitStatus
                            ? 'Not Started'
                            : 'Not Started (tap to choose status)'
                        }
                        value="Not Started"
                      />
        ...
        {allCompleted && (
          <View
            style={tw`p-4 rounded-xl bg-[#eef7ff] dark:bg-[#0b1724]`}
          >
            <Text
              style={tw`text-sm text-[#0d141c] dark:text-white`}
            >
              🎉 Nice work! You’ve completed every week. Check your
              Achievements page for badges.
            </Text>
              ...
              {!!oerMeta ? (
                <>
                  <TouchableOpacity
                    ...
                    style={tw.style(
                      `rounded-xl h-10 px-4 bg-emerald-600 dark:bg-emerald-600 justify-center`,
                  ...
                  <TouchableOpacity
                    ...
                    disabled={downloadingTranscript || !courseWatchedAll}
                    style={tw.style(
                      `rounded-xl h-10 px-4 bg-indigo-600 dark:bg-indigo-600 justify-center`,
                      (!courseWatchedAll || downloadingTranscript) &&
                        `opacity-60`,
                    )}
                  >
                    <Text
            ...
            {!oerMeta && !hasMyReview && (
              <TouchableOpacity
                onPress={() => setOpenReview(true)}
                style={tw`mt-3 rounded-xl h-10 px-4 bg-[#e7edf4] dark:bg-[#172534] justify-center self-start`}
              >
                <Text
                  style={tw`text-sm font-semibold text-[#0d141c] dark:text-white`}
                >
        ...
        >
          <View
            style={tw`flex-1 items-center justify-center bg-black/40 px-4`}
          >
            <View
              style={tw`w-full max-w-md rounded-2xl bg-white dark:bg-[#0b1016] p-4 border border-[#cedbe8] dark:border-white/10`}
            >
              <Text
                style={tw`text-lg font-bold mb-2 text-[#0d141c] dark:text-white`}
              >
                Rate this course
              </Text>
              ...
              <TextInput
                ...
                maxLength={500}
                multiline
                style={tw`w-full text-sm rounded-lg p-2 bg-[#e7edf4] dark:bg-[#172534] text-[#0d141c] dark:text-white min-h-24`}
              />
              ...
              <View style={tw`mt-4 flex-row items-center gap-2`}>
                <TouchableOpacity
                  ...
                  style={tw.style(
                    `px-4 h-10 rounded-xl bg-[#3d99f5] dark:bg-[#3d99f5] justify-center`,
                ...
                <TouchableOpacity
                  ...
                  style={tw`px-4 h-10 rounded-xl bg-white dark:bg-[#0b1016] border border-[#cedbe8] dark:border-white/10 justify-center`}
                >
                  <Text
                    style={tw`text-sm font-semibold text-[#0d141c] dark:text-white`}
                  >
