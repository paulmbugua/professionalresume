...
const BAR = {
  rowH: 'h-9', // compact top row
  pill: 'h-9', // room for inner padding
};

const PILL_ITEMS: { label: string; route: keyof MainStackParamList }[] = [
  { label: 'Find Tutor',       route: 'FindTutor' },
  { label: 'My Courses',       route: 'Courses' },
  { label: 'ClassVault',       route: 'ClassVaultLibrary' },
  { label: 'Resources',        route: 'Resources' },
  { label: 'Messages',         route: 'Messages' },
  { label: 'For Institutions', route: 'InstitutionLogin' }, // or 'OrgElearnPortal'
];
...
const NavbarNative: React.FC<Props> = ({ onSearch }) => {
  ...
  return (
    <View style={tw`bg-white/75 dark:bg-[#0b121a]/75 border-b border-gray-200 dark:border-darkCard pt-2 pb-2`}>
      {/* Compact search row (profile icon removed) */}
      <View style={tw`${BAR.rowH} px-3 flex-row items-center mt-0.5`}>
        <View
          style={tw`flex-1 flex-row items-center rounded-xl px-3 ${BAR.pill}
        ...
        >
          ...
          <TextInput
            ...
            // roomy + faint text inside the field
      ...
      {/* Thin pills row — with a small gap below the search bar */}
      <ScrollView
        horizontal
